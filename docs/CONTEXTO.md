...
**Clínica** *(mais novo papel, 2026-07-23)*: onboarding espelhando médico (+ verificação automática de CNPJ via BrasilAPI), busca o mesmo catálogo de médicos/horários, preenche dados do tutor+pet, `signInWithOtp` cria/acha a conta do tutor sem Edge Function, RPC `clinica_criar_agendamento` cria o agendamento em nome dele com split de preço correto; tutor recebe magic link e confirma/paga. Endereço do exame hoje é **sempre** o endereço residencial do tutor (não existe opção "na própria clínica" — ver [[animagem-feature-ideas]] / `docs/PENDENCIAS.md`).
...
O catálogo (`exames_catalogo`, seed original: Ultrassom Abdominal/Gestacional, Ecocardiograma, Ultrassom Ortopédico/Ocular, Raio-X, Eletrocardiograma) e toda a busca do tutor (`buscar()`, `renderCards`) foram desenhados assumindo que o médico oferece "exames" de imagem/diagnóstico. Com o novo posicionamento (qualquer especialidade), isso precisa evoluir — mas nenhuma mudança de schema foi feita ainda para isso, é só uma direção de produto confirmada pelo André. Não assumir que "exame" já cobre "consulta com clínico geral" no código atual.

## 7. Onde ver o que falta

Pendências técnicas priorizadas: [`PENDENCIAS.md`](PENDENCIAS.md) — no momento desta revisão, os temas ativos de trabalho são monitoramento de erros, completar o papel Clínica (dados bancários para repasse, testar a tela admin no navegador, e adicionar escolha de local do exame: casa do tutor vs. na própria clínica) e marca/identidade visual.
